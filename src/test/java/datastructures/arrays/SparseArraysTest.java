...
public class SparseArraysTest {
    ...
    static int[] matchingStrings2(String[] strings, String[] queries) {
        ...
        return Arrays.stream(queries).map(q -> Arrays.stream(strings).filter(s -> s.matches(q)).count()).mapToInt(Long::intValue).toArray();

    }

    static int[] matchingStrings3(String[] strings, String[] queries) {

        Map<String, Long> groupedStringsMap =
        ...
        return Arrays.stream(queries).map(q -> groupedStringsMap.getOrDefault(q, 0L)).mapToInt(Long::intValue).toArray();
    }
    ...
    void matchingStrings1Test() {
        assertArrayEquals(new int[]{2, 1, 0}, matchingStrings1(new String[]{"aba", "baba", "aba", "xzxb"}, new String[]{"aba", "xzxb", "ab"}));
    }

    @Test
    void matchingStrings2Test() {
        assertArrayEquals(new int[]{2, 1, 0}, matchingStrings2(new String[]{"aba", "baba", "aba", "xzxb"}, new String[]{"aba", "xzxb", "ab"}));
    }

    @Test
    void matchingStrings3Test() {
        assertArrayEquals(new int[]{2, 1, 0}, matchingStrings3(new String[]{"aba", "baba", "aba", "xzxb"}, new String[]{"aba", "xzxb", "ab"}));
    }
    ...
    void matchingStrings4Test() {
        ...
        String[] strings = Stream.generate(SparseArraysTest::generateRandomStringInStream).limit(1000).toArray(String[]::new);
        String[] queries = Stream.generate(SparseArraysTest::generateRandomStringInStream).limit(1000).toArray(String[]::new);
        ...
        System.out.printf("test1, result time %d ms - result: %s%n", Duration.between(start, finish).toMillis(), intArrayToString(result));


        start = Instant.now();
        result = matchingStrings2(strings, queries);
        finish = Instant.now();
        System.out.printf("test2, result time %d ms - result: %s%n", Duration.between(start, finish).toMillis(), intArrayToString(result));

        start = Instant.now();
        result = matchingStrings3(strings, queries);
        finish = Instant.now();
        System.out.printf("test3, result time %d ms - result: %s%n", Duration.between(start, finish).toMillis(), intArrayToString(result));
    ...
    void intArrayToStringTestClasic() {
        int[] testInts = {1, 2, 2, 2, 2, 2, 2, 2, 22, 3, 2, 22, 2, 2, 2, 2, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 5, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3, 3};
